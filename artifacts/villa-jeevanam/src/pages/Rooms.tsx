...
import { Info, Clock, CreditCard, AlertTriangle, CalendarX, Ban, CloudLightning } from "lucide-react";
...
const rooms = [
  {
    ...
    features: ["Queen Bed", "En-suite Bathroom", "Garden View", "Air Conditioning", "32\" Smart TV"],
  },
  {
    id: 2,
...
const PLANS = [
  { label: "EP", fullName: "European Plan", desc: "Room only — no meals included", price: 2300 },
  { label: "CP", fullName: "Continental Plan", desc: "Room + Breakfast", price: 2800 },
  { label: "MAP", fullName: "Modified American Plan", desc: "Room + Breakfast + Dinner", price: 3500 },
  { label: "AP", fullName: "American Plan", desc: "Room + All Meals (Breakfast, Lunch & Dinner)", price: 4000 },
];
    ...
    <Layout>
      ...
      <div className="relative pt-32 pb-16 px-4 bg-background overflow-hidden">
        <div className="absolute inset-0 z-0 opacity-10">
          <div
            ...
            style={{ backgroundImage: `url('https://images.unsplash.com/photo-1582719478250-c89fae46583b?w=1920&q=80')` }}
          />
          <div className="absolute inset-0 bg-background/80" />
        </div>
        <div className="max-w-7xl mx-auto relative z-10 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <p className="text-primary text-sm tracking-[0.2em] uppercase mb-3">Accommodations</p>
            <h1 className="text-4xl md:text-6xl font-serif font-bold text-foreground mb-4">Our Rooms</h1>
            <p className="text-foreground/60 max-w-xl mx-auto">
              Each room is thoughtfully designed to bring comfort, elegance, and a touch of nature to your stay.
            </p>
          </motion.div>
        </div>
      </div>
          ...
          {rooms.map((room, i) => (
            ...
            >
              ...
              <div className="lg:w-1/2 p-8 md:p-12 flex flex-col justify-center">
                <p className="text-primary text-xs tracking-widest uppercase mb-2">Room {room.id < 10 ? `0${room.id}` : room.id}</p>
                <h3 className="text-2xl md:text-3xl font-serif text-foreground mb-4 group-hover:text-primary transition-colors">
                  {room.title}
                </h3>
                <p className="text-foreground/65 leading-relaxed mb-6">{room.description}</p>
                <div className="flex flex-wrap gap-2 mb-8">
                  {room.features.map(feat => (
                    <span key={feat} className="text-xs px-3 py-1.5 bg-secondary rounded-full text-foreground/80 border border-border">
                      {feat}
                    </span>
                  ))}
                </div>
                <Link href="/contact">
                  <Button className="w-full sm:w-auto">Reserve This Room</Button>
                </Link>
              </div>
            </motion.div>
          ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
            {PLANS.map((plan, i) => (
              ...
              >
                ...
                <p className="text-foreground/40 text-xs mt-1">per room / night</p>
              </motion.div>
            ))}
          </div>

          {/* Pricing Table */}
          <motion.div {...fadeUp(0.1)} className="rounded-2xl overflow-hidden border border-border/50 shadow-xl">
            <div className="bg-card/80 px-6 py-4 border-b border-border/50 flex items-center gap-3">
              <div className="w-2 h-2 rounded-full bg-primary" />
              <h3 className="font-serif text-lg text-foreground">Standard Room — Per Night Rates</h3>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border/30">
                  ...
                  <tr className="bg-card/60 hover:bg-secondary/20 transition-colors">
                    <td className="px-6 py-4 text-foreground/80">Extra Bed <span className="block text-foreground/40 text-xs">Child 5–12 yrs</span></td>
                  ...
                  <tr className="bg-card hover:bg-secondary/20 transition-colors">
                    <td className="px-6 py-4 text-foreground/80">Child Without Bed <span className="block text-foreground/40 text-xs">5–12 yrs</span></td>
                  ...
                  <tr className="bg-card/60 hover:bg-secondary/20 transition-colors">
                    <td className="px-6 py-4 text-foreground/80">Child Below 5 Yrs</td>
                    <td colSpan={4} className="px-6 py-4 text-center">
                      <span className="inline-block bg-primary/10 text-primary text-xs font-medium px-3 py-1 rounded-full border border-primary/20">Complimentary</span>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div className="bg-secondary/30 border-t border-border/50 px-6 py-5 flex flex-col sm:flex-row gap-4 sm:gap-8 text-xs text-foreground/60">
              <div className="flex items-center gap-2"><Info size={14} className="text-primary shrink-0" /><span>Applicable GST extra on all rates</span></div>
              <div className="flex items-center gap-2"><Clock size={14} className="text-primary shrink-0" /><span>Check-in / Check-out: 08:00 AM</span></div>
              <div className="flex items-center gap-2"><CreditCard size={14} className="text-primary shrink-0" /><span>Valid government ID required at check-in</span></div>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20 px-4 bg-background">
        <div className="max-w-6xl mx-auto">
          ...
          <motion.div {...fadeUp(0.3)} className="mt-8 p-5 rounded-xl bg-primary/5 border border-primary/15 flex items-start gap-3">
      ...
      <section className="py-16 px-4 bg-secondary/20 border-t border-border/40 text-center">
        <motion.div {...fadeUp(0)}>
          <h2 className="text-3xl font-serif font-bold text-foreground mb-4">Ready to book your room?</h2>
          <p className="text-foreground/60 mb-8 max-w-md mx-auto">Choose your dates and plan, and submit your reservation request today.</p>
          <Link href="/contact">
            <Button size="lg" className="px-12">Make a Reservation</Button>
          </Link>
        </motion.div>
      </section>
    </Layout>
